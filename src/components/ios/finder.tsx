...
export function FinderApp() {
  ...
  const allLocations = [locations.work, ...locations.work.children]
    ...
    <div className="h-full w-full bg-gray-50 flex flex-col text-black">
      ...
      <div className="flex-1 overflow-auto">
        <div className="px-4 py-4">
          ...
          <h2 className="text-xl font-bold mt-8 mb-3 px-1">Favorites</h2>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            {allLocations.map((item: any, idx: number) => (
              <div 
                key={item.id}
                className={`flex items-center gap-3 p-4 ${idx !== allLocations.length - 1 ? 'border-b border-gray-100' : ''} active:bg-gray-50 cursor-pointer`}
                onClick={() => {
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  {item.fileType && <p className="text-xs text-gray-400 mt-0.5">{item.fileType.toUpperCase()} File</p>}
                </div>
                <ChevronRight className="text-gray-400" size={20} />
              </div>
            ))}
